\hypertarget{md_userman_autotoc_md9}{}\doxysection{A program általános leírása}\label{md_userman_autotoc_md9}
...
\item Infectious(Fertőző)\+: A már megfertőzött személyeket jelképezik
\item Healthy(Egészséges)\+: A már fertőző pöttyök képesek a gyógyulásra előre beállított eséllyel
\item Dead(\+Halott)\+: A már fertőző pöttyök képesek a meghalásra előre beállított eséllyel
\end{DoxyItemize}
...
Egy fertőzött pötty másokat tovább tud fertőzni, illetve a lappangási idő letelte után (500 tick), lehetősége lesz gyógyulni, illetve meghalni a beállított valószínűség alapján.\hypertarget{md_userman_autotoc_md10}{}\doxysection{A program funkcionalitása a felhasználó szemszögéből}\label{md_userman_autotoc_md10}
...
\item Pötty sebessége(\+Speed slid\+Wer)
\item n db pötty felhelyezése véletlenszerűen a pályára (Add button)
\item pöttyök egyessével történő felhelyezése a pályára, egér kattintás alapján (4db button)
...
Ha a szimuláció már el lett indítva 10 Tickenként a pillanatnyi állás elküldésre kerül a grafikonhoz. A grafikon 100 ms -\/ként frissül. Ha a statisztika bezárásra kerül, akkor az addig kapott adatok törlődnek.
...
\begin{DoxyItemize}
\item Olyan diagram(tick szerint), ahol ábrázolva vannak a populáció, a fertőzöttek, a gyógyultak és a halottak 
\end{DoxyItemize}
\end{DoxyItemize}